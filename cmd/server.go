	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if conf.Conf.Scheme.HttpPort != -1 {
			...
			httpSrv = &http.Server{
				Addr:              httpBase,
				Handler:           httpHandler,
				ReadTimeout:       time.Duration(setting.GetInt(conf.HTTPServerReadTimeout, 0)) * time.Second,
				WriteTimeout:      time.Duration(setting.GetInt(conf.HTTPServerWriteTimeout, 0)) * time.Second,
				IdleTimeout:       time.Duration(setting.GetInt(conf.HTTPServerIdleTimeout, 120)) * time.Second,
				ReadHeaderTimeout: time.Duration(setting.GetInt(conf.HTTPServerReadHeaderTimeout, 30)) * time.Second,
				MaxHeaderBytes:    setting.GetInt(conf.HTTPServerMaxHeaderBytes, 1<<20),
			}
			go func() {
